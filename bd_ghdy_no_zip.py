...
class Ghdy:
 ...
 def get_id(self):
  try:
   ...
   signature=sha256.hexdigest()
   headers={'X-SESSION-ID':self.session,'X-REQUEST-ID':f'{request}','X-TIMESTAMP':f'{current_timestamp}','X-SIGNATURE':f'{signature}','X-TENANT-ID':'49','User-Agent':'5.0.7.0.0;00000000-699e-0680-0000-000055f72c53;Xiaomi Redmi Note 8 Pro;Android;11;Release','X-ACCOUNT-ID':self.account,'Cache-Control':'no-cache','Host':'vapp.tmuyun.com','Connection':'Keep-Alive','Accept-Encoding':'gzip'}
   params={'channel_id':'6254f12dfe3fc10794f7b25c','isDiFangHao':'false','is_new':'true','list_count':'0','size':'20'}
 ...
 def look(self):
  try:
   for params_id in self.id_list[:6]:
    ...
    headers={'X-SESSION-ID':self.session,'X-REQUEST-ID':f'{request}','X-TIMESTAMP':f'{current_timestamp}','X-SIGNATURE':f'{signature}','X-TENANT-ID':'49','User-Agent':'5.0.7.0.0;00000000-699e-0680-0000-000055f72c53;Xiaomi Redmi Note 8 Pro;Android;11;Release','X-ACCOUNT-ID':self.session,'Cache-Control':'no-cache','Host':'vapp.tmuyun.com','Connection':'Keep-Alive','Accept-Encoding':'gzip'}
    params={'id':params_id}
 ...
 def chou(self):
  try:
   ...
   headers={'Host':'fijdzpur.act.tmuact.com','Connection':'keep-alive','Pragma':'no-cache','Cache-Control':'no-cache','Accept':'application/json, text/plain, */*','X-Requested-With':'XMLHttpRequest','User-Agent':'Mozilla/5.0 (Linux; Android 11; Redmi Note 8 Pro Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36;xsb_dongyang;xsb_dongyang;5.0.7.0.0;native_app','Content-Type':'application/x-www-form-urlencoded','Origin':'https://fijdzpur.act.tmuact.com','Sec-Fetch-Site':'same-origin','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty','Referer':'https://fijdzpur.act.tmuact.com/money/index/index.html','Accept-Language':'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7'}
   data={'m':'front','subm':'money','action':'open','account_id':self.account,'session_id':self.session,'token':'','q':'YunSLfAkU'}
 ...
 def tx(self):
  ...
  data={'m':'front','subm':'money_wallet','action':'commonchange','account_id':self.account,'session_id':self.session,'app':'XSB_DONGYANG'}
  h={'Host':'fijdzpur.act.tmuact.com','Connection':'keep-alive','Pragma':'no-cache','Cache-Control':'no-cache','Accept':'application/json, text/plain, */*','X-Requested-With':'XMLHttpRequest','User-Agent':'Mozilla/5.0 (Linux; Android 11; Redmi Note 8 Pro Build/RP1A.200720.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/87.0.4280.141 Mobile Safari/537.36;xsb_dongyang;xsb_dongyang;5.0.7.0.0;native_app','Content-Type':'application/x-www-form-urlencoded','Origin':'https://fijdzpur.act.tmuact.com','Sec-Fetch-Site':'same-origin','Sec-Fetch-Mode':'cors','Sec-Fetch-Dest':'empty','Referer':'https://fijdzpur.act.tmuact.com/money/index/index.html','Accept-Language':'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7'}
